...
import { Folder, FolderInput, FolderPlus, Import, Loader2 } from 'lucide-react'
...
const V1_STEPS: ReactNode[] = [
  ...
  <>
    Click <Emphasis>Open exported folder</Emphasis> below and select it — or drag the
    folder (or the <Emphasis>.zip</Emphasis>) straight onto this window.
  </>,
]
 ...
 *
 * A Reflect export (`.zip` or folder) can also be dropped anywhere on this
 * screen to import it as a new graph under Documents (see {@link useGraphImport}).
 *
...
export function GraphChooser(): ReactElement {
  ...
  // picker/recent failure (provider `error`) is never hidden behind it.
      ...
      <div className="m-auto w-full max-w-2xl space-y-8">
        ...
        <div className="grid items-stretch gap-4 sm:grid-cols-2">
          ...
          <section className="flex flex-col gap-4 rounded-xl border border-border bg-surface p-5 shadow-sm">
            ...
            <Button type="button" className="mt-auto w-full" onClick={openPicker}>
              <FolderPlus aria-hidden strokeWidth={1.75} />
              Choose a folder…
            </Button>
          </section>

          {/* Coming from Reflect V1 — guided export → unzip → open (or drag in). */}
          <section className="flex flex-col gap-4 rounded-xl border border-border bg-surface p-5 shadow-sm">
            <div className="space-y-1.5">
            ...
            <Button type="button" variant="outline" className="mt-auto w-full" onClick={openPicker}>
 ...
 * chooser. Pointer-events stay off so the drag's heartbeat still resolves
